...
import { type AdvancedTableActionId } from './actions';
import { AdvancedTablesFloatingControls } from './floating-controls';
import {
  defaultSettings,
  TableEditorPluginSettings,
  type AdvancedTablesEmbeddedSettingsData,
} from './settings';
import {
  TableControlsView,
...
export default class EmbeddedAdvancedTablesPlugin extends Plugin {
  public settings: TableEditorPluginSettings;
  private lastMarkdownView: MarkdownView | null = null;
  private floatingControls: AdvancedTablesFloatingControls | null = null;

  public onload(): Promise<void> {
    return this.asyncOnload();
  }

  private async asyncOnload(): Promise<void> {
    await this.loadSettings();

    this.resolveMarkdownViewForTableAction();
    const workspace = this.app.workspace as any;
    if (typeof workspace.on === 'function') {
      this.registerEvent(
        workspace.on('active-leaf-change', (leaf: any) => {
    ...
    this.floatingControls = new AdvancedTablesFloatingControls({
      app: this.app,
      runAction: (actionId) => this.runAdvancedTableActionForManager(actionId),
      rememberView: (view) => {
        this.rememberMarkdownView(view);
      },
      getShowFloatingControls: () => this.settings.showFloatingControls,
      getShowColorBlocks: () => this.settings.showColorBlocks,
      getShowZebraStripes: () => this.settings.showZebraStripes,
      setShowColorBlocks: async (value) => {
        this.settings.showColorBlocks = value;
        await this.saveData(this.settings as AdvancedTablesEmbeddedSettingsData);
        this.floatingControls?.updateFromSettings();
      },
      setShowZebraStripes: async (value) => {
        this.settings.showZebraStripes = value;
        await this.saveData(this.settings as AdvancedTablesEmbeddedSettingsData);
        this.floatingControls?.updateFromSettings();
      },
    });
    this.floatingControls.start();
    this.register(() => {
    ...
    if (this.settings.showRibbonIcon) {
      this.addRibbonIcon('spreadsheet', '成熟版表格工具栏', () => {
    ...
    this.addCommand({
      ...
      editorCheckCallback: this.newPerformTableAction((te: TableEditor) => {
        if (this.settings.bindEnter && !this.isMobile()) {
          new Notice('成熟版表格增强：回车键已绑定跳到下一行，如出现重复动作，可在设置中关闭。');
        }
        te.nextRow();
      }),
    });
    ...
    this.addCommand({
      ...
      editorCheckCallback: this.newPerformTableAction((te: TableEditor) => {
        if (this.settings.bindTab && !this.isMobile()) {
          new Notice('成熟版表格增强：制表键已绑定跳到下一个单元格，如出现重复动作，可在设置中关闭。');
        }
        te.nextCell();
      }),
    });
    ...
    this.addCommand({
      ...
      editorCheckCallback: this.newPerformTableAction((te: TableEditor) => {
        if (this.settings.bindTab && !this.isMobile()) {
          new Notice('成熟版表格增强：反向跳格快捷键已启用，如出现重复动作，可在设置中关闭。');
        }
        te.previousCell();
      }),
    });
    ...
    (fn: (te: TableEditor) => void): (() => boolean) =>
      (): boolean => {
        try {
          ...
          console.warn('成熟版表格增强：快捷键处理失败', error);
        }
        return false;
      };
    ...
    (fn: (te: TableEditor) => void) =>
      (checking: boolean, editor: Editor, view: MarkdownView): boolean | void => {
        try {
          ...
          if (!checking) {
            console.warn('成熟版表格增强：表格操作失败', error);
            new Notice('表格操作失败，请确认光标在 Markdown 表格中。');
          }
          return false;
        }
      };
  ...
  public getAdvancedTableSettingsForManager(): AdvancedTableSettingsForManager {
    return {
      bindEnter: this.settings.bindEnter,
      bindTab: this.settings.bindTab,
      formatType: this.settings.formatType === FormatType.WEAK ? 'weak' : 'normal',
      showRibbonIcon: this.settings.showRibbonIcon,
      showFloatingControls: this.settings.showFloatingControls,
      showColorBlocks: this.settings.showColorBlocks,
      showZebraStripes: this.settings.showZebraStripes,
    };
  }

  public async updateAdvancedTableSettingsFromManager(
    input: Partial<AdvancedTableSettingsForManager>,
  ): Promise<AdvancedTableSettingsForManager> {
    if (typeof input.bindEnter === 'boolean') this.settings.bindEnter = input.bindEnter;
    if (typeof input.bindTab === 'boolean') this.settings.bindTab = input.bindTab;
    if (typeof input.showRibbonIcon === 'boolean') this.settings.showRibbonIcon = input.showRibbonIcon;
    if (typeof input.showFloatingControls === 'boolean') this.settings.showFloatingControls = input.showFloatingControls;
    if (typeof input.showColorBlocks === 'boolean') this.settings.showColorBlocks = input.showColorBlocks;
    if (typeof input.showZebraStripes === 'boolean') this.settings.showZebraStripes = input.showZebraStripes;
    if (input.formatType === 'weak' || input.formatType === 'normal') {
      this.settings.formatType = input.formatType === 'weak' ? FormatType.WEAK : FormatType.NORMAL;
    }
    await this.saveData(this.settings as AdvancedTablesEmbeddedSettingsData);
    this.floatingControls?.updateFromSettings();
    return this.getAdvancedTableSettingsForManager();
  }

  public async runAdvancedTableActionForManager(commandId: AdvancedTableActionId): Promise<boolean> {
    if (commandId === 'table-control-bar') {
      await this.toggleTableControlsView();
      return true;
    }

    const view = this.resolveMarkdownViewForTableAction();
    if (!view?.file || !view.editor) {
      new Notice('请先打开一篇 Markdown 笔记，并把光标放到表格里。');
      return false;
    }

    if (typeof (view as any).getMode === 'function' && (view as any).getMode() !== 'source') {
      new Notice('请先切到编辑视图后再操作 Markdown 表格。');
      return false;
    }

    try {
      const te = new TableEditor(this.app, view.file, view.editor, this.settings);
      if (commandId === 'format-all-tables') {
        te.formatAllTables();
        return true;
      }

      if (commandId === 'evaluate-formulas') {
        if (!te.cursorIsInTable() && !te.cursorIsInTableFormula()) {
          new Notice('请先把光标放到 Markdown 表格或公式里。');
          return false;
        }
        te.evaluateFormulas();
        return true;
      ...
      console.warn('成熟版表格增强：设置页操作失败', error);
      new Notice('表格操作失败，请确认光标在 Markdown 表格中。');
      return false;
    }
  }
  ...
  private rememberMarkdownView(view: unknown): MarkdownView | null {
    const candidate = view as MarkdownView | null | undefined;
    if (!candidate?.file || !candidate.editor) return null;
    this.lastMarkdownView = candidate;
    return candidate;
  }

  private resolveMarkdownViewForTableAction(): MarkdownView | null {
    const active = this.rememberMarkdownView(
      this.app.workspace.getActiveViewOfType(MarkdownView),
    );
    if (active) return active;

    const activeFile = (this.app.workspace as any).getActiveFile?.();
    const leaves = this.app.workspace.getLeavesOfType('markdown');
    const rememberedIsOpen = !!this.lastMarkdownView && (
      leaves.length === 0 || leaves.some((leaf) => leaf.view === this.lastMarkdownView)
    );
    if (rememberedIsOpen) return this.lastMarkdownView;

    const matchingActiveFile = leaves.find((leaf) => {
      const view = leaf.view as MarkdownView;
      return !!activeFile?.path && view?.file?.path === activeFile.path;
    });
    const fromLeaves = this.rememberMarkdownView(
      matchingActiveFile?.view ?? leaves.find((leaf) => !!(leaf.view as MarkdownView)?.file)?.view,
    );
...
type AdvancedTableSettingsForManager = {
  bindTab: boolean;
  bindEnter: boolean;
...
class TableEditorSettingsTab extends PluginSettingTab {
  private readonly plugin: EmbeddedAdvancedTablesPlugin;

  constructor(app: App, plugin: EmbeddedAdvancedTablesPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  public display(): void {
    ...
    new Setting(containerEl)
      .setName('回车键跳到下一行')
      .setDesc(
        '需要重载插件后生效。开启后，光标在表格中按回车键会跳到下一行；如果和中文输入法或补全冲突，可以关闭。',
      )
      .addToggle((toggle) =>
    ...
    new Setting(containerEl)
      .setName('制表键跳格')
      .setDesc(
        '需要重载插件后生效。开启后，光标在表格中可用制表键和反向跳格在单元格之间移动；如果和输入法或补全冲突，可以关闭。',
      )
      .addToggle((toggle) =>
    ...
    new Setting(containerEl)
      .setName('用空格对齐源码列（高级）')
      .setDesc(
        '开启后会在 Markdown 源码中写入大量填充空格。默认关闭，实时预览下无需开启。',
      )
      .addToggle((toggle) =>
        toggle
    ...
    new Setting(containerEl)
      .setName('显示侧栏图标')
      .setDesc(
        '开启后会在侧边栏显示成熟版表格工具栏入口；关闭后仍可从总插件设置页管理。改动需要重载插件后生效。',
      )
      .addToggle((toggle) =>
        toggle
