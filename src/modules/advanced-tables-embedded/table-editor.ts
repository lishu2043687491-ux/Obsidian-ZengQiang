...
export class TableEditor {
  ...
  public readonly evaluateFormulas = (): void => {
    ...
    if (err) {
      new Notice(`公式计算失败：${translateAdvancedTablesError(err.message)}`);
    }
  };
...
function translateAdvancedTablesError(message: string) {
  const normalized = String(message || "").trim();
  const known: Record<string, string> = {
    "Empty table": "空表格",
    "Unknown alignment": "未知对齐方式",
    "Unexpected default alignment": "默认对齐方式异常",
    "Unknown format type": "未知格式化方式",
  };
  return known[normalized] ?? "请检查公式或表格格式";
}
